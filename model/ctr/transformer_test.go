...
func TestAutoScaler_ConstantValues(t *testing.T) {
	t.Run("constant values skip scaling", func(t *testing.T) {
		scaler := NewAutoScaler()
		values := []float32{5, 5, 5, 5, 5}
		scaler.Fit(values)

		assert.True(t, scaler.Skip)
		// Should return original value when skipped
		assert.Equal(t, float32(5), scaler.Transform(5))
		assert.Equal(t, float32(10), scaler.Transform(10))
	})

	t.Run("near-constant values skip scaling", func(t *testing.T) {
		scaler := NewAutoScaler()
		// Variance = (0.0001^2 + 0 + 0 + 0 + 0.0001^2) / 5 ≈ 2e-9 < 1e-6
		values := []float32{5.0, 5.0001, 5.0, 5.0, 4.9999}
		scaler.Fit(values)

		assert.True(t, scaler.Skip)
	})

	t.Run("varying values do not skip scaling", func(t *testing.T) {
		scaler := NewAutoScaler()
		values := []float32{0, 10, 100, 1000}
		scaler.Fit(values)

		assert.False(t, scaler.Skip)
		assert.True(t, scaler.UseLog)
	})

	t.Run("single value skips scaling", func(t *testing.T) {
		scaler := NewAutoScaler()
		values := []float32{42}
		scaler.Fit(values)

		assert.True(t, scaler.Skip)
		assert.Equal(t, float32(42), scaler.Transform(42))
	})

	t.Run("empty values skips scaling", func(t *testing.T) {
		scaler := NewAutoScaler()
		values := []float32{}
		scaler.Fit(values)

		assert.False(t, scaler.Skip) // Skip remains false (default)
	})

	t.Run("marshal and unmarshal with skip", func(t *testing.T) {
		scaler := NewAutoScaler()
		values := []float32{10, 10, 10}
		scaler.Fit(values)
		...
		assert.Equal(t, scaler.Skip, scaler2.Skip)
		assert.Equal(t, scaler.Transform(100), scaler2.Transform(100))
	})
}
