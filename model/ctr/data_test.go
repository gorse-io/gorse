...
func TestDataset_ComputeWeights(t *testing.T) {
	t.Run("no weight config", func(t *testing.T) {
		dataset := &Dataset{
			FeedbackTypes:  []string{"click", "purchase"},
			FeedbackValues: []float64{1.0, 1.0},
		}
		err := dataset.ComputeWeights(nil)
		assert.NoError(t, err)
		assert.Nil(t, dataset.SampleWeights)
	})
	...
	t.Run("empty weight config", func(t *testing.T) {
		...
		err := dataset.ComputeWeights(map[string]string{})
		assert.NoError(t, err)
		assert.Nil(t, dataset.SampleWeights)
	})
	...
	t.Run("no feedback types", func(t *testing.T) {
		dataset := &Dataset{}
		err := dataset.ComputeWeights(map[string]string{"click": "1"})
		assert.NoError(t, err)
		assert.Nil(t, dataset.SampleWeights)
	})
	...
	t.Run("constant weights", func(t *testing.T) {
		dataset := &Dataset{
			FeedbackTypes:  []string{"click", "purchase", "click"},
			FeedbackValues: []float64{1.0, 1.0, 1.0},
		}
		err := dataset.ComputeWeights(map[string]string{
			"click":    "1",
			"purchase": "5",
		...
		assert.NoError(t, err)
		assert.NotNil(t, dataset.SampleWeights)
		assert.Equal(t, float32(1.0), dataset.SampleWeights[0])
		assert.Equal(t, float32(5.0), dataset.SampleWeights[1])
		assert.Equal(t, float32(1.0), dataset.SampleWeights[2])
	})

	t.Run("value-based weights", func(t *testing.T) {
		dataset := &Dataset{
			FeedbackTypes:  []string{"rating", "rating", "rating"},
			FeedbackValues: []float64{3.0, 4.0, 5.0},
		}
		err := dataset.ComputeWeights(map[string]string{
			"rating": "Value",
		})
		assert.NoError(t, err)
		assert.NotNil(t, dataset.SampleWeights)
		assert.Equal(t, float32(3.0), dataset.SampleWeights[0])
		assert.Equal(t, float32(4.0), dataset.SampleWeights[1])
		assert.Equal(t, float32(5.0), dataset.SampleWeights[2])
	})

	t.Run("expression weights", func(t *testing.T) {
		dataset := &Dataset{
			FeedbackTypes:  []string{"view_time", "view_time", "view_time"},
			FeedbackValues: []float64{99.0, 999.0, 9.0},
		}
		err := dataset.ComputeWeights(map[string]string{
		...
		assert.NoError(t, err)
		assert.NotNil(t, dataset.SampleWeights)
		assert.InDelta(t, float32(math.Log1p(99.0)), dataset.SampleWeights[0], 0.001)
		assert.InDelta(t, float32(math.Log1p(999.0)), dataset.SampleWeights[1], 0.001)
		assert.InDelta(t, float32(math.Log1p(9.0)), dataset.SampleWeights[2], 0.001)
	})

	t.Run("unknown feedback type uses default", func(t *testing.T) {
		dataset := &Dataset{
			FeedbackTypes:  []string{"click", "unknown", "purchase"},
			FeedbackValues: []float64{1.0, 1.0, 1.0},
		}
		err := dataset.ComputeWeights(map[string]string{
			"click":    "1",
		...
		assert.NoError(t, err)
		assert.Equal(t, float32(1.0), dataset.SampleWeights[0])
		assert.Equal(t, float32(1.0), dataset.SampleWeights[1]) // unknown type
		assert.Equal(t, float32(5.0), dataset.SampleWeights[2])
	})

	t.Run("invalid expression returns error", func(t *testing.T) {
		dataset := &Dataset{
			FeedbackTypes:  []string{"click"},
			FeedbackValues: []float64{1.0},
		}
		err := dataset.ComputeWeights(map[string]string{
			"click": "invalid++",
		})
