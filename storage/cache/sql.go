...
func (db *SQLDatabase) UpdateScores(ctx context.Context, collections []string, subset, id string, patch ScorePatch) error {
	...
	tx := db.gormDB.WithContext(ctx).Model(&PostgresDocument{}).
		Where("collection in (?) and id = ? and (subset = ? or ? = '')",
