...
func TestMaster_LoadDataFromDatabase(t *testing.T) {
	...
	// check latest items
	latest, err := m.CacheClient.GetSort(cache.Key(cache.LatestItems, ""), 0, 100)
	assert.NoError(t, err)
	...
	latest, err = m.CacheClient.GetSort(cache.Key(cache.LatestItems, "2"), 0, 100)
	assert.NoError(t, err)
	assert.Equal(t, []cache.Scored{
		{items[8].ItemId, float32(items[8].Timestamp.Unix())},
	...
	// check popular items
	popular, err := m.CacheClient.GetSort(cache.Key(cache.PopularItems, ""), 0, 2)
	assert.NoError(t, err)
	assert.Equal(t, []cache.Scored{
	...
	popular, err = m.CacheClient.GetSort(cache.Key(cache.PopularItems, "2"), 0, 2)
	assert.NoError(t, err)
	assert.Equal(t, []cache.Scored{
		{Id: items[8].ItemId, Score: 9},
