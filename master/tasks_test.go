...
func TestRunCacheGarbageCollectionTask(t *testing.T) {
	...
	s, err = m.CacheClient.Get(cache.Key(cache.ItemNeighborsDigest, "20")).String()
	assert.True(t, errors.IsNotFound(err))
	ts, err = m.CacheClient.Get(cache.Key(cache.LastModifyItemTime, "20")).Time()
	assert.True(t, errors.IsNotFound(err))
	ts, err = m.CacheClient.Get(cache.Key(cache.LastUpdateItemNeighborsTime, "20")).Time()
	assert.True(t, errors.IsNotFound(err))
